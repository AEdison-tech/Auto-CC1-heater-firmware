...
function Settings() {
  ...
  const [activationTemp, setActivationTemp] = createSignal(30.0)
  const [controlSource, setControlSource] = createSignal('heater')
  const [requirePrinting, setRequirePrinting] = createSignal(false)
  const [targetTemp, setTargetTemp] = createSignal(35.0)
  const [hysteresis, setHysteresis] = createSignal(2.0)
  ...
  onMount(async () => {
    try {
      ...
      setActivationTemp(s.activation_temp ?? 30.0)
      setControlSource(s.control_source ?? 'heater')
      setRequirePrinting(s.require_printing ?? false)
      setTargetTemp(s.target_temp ?? 35.0)
      setHysteresis(s.hysteresis ?? 2.0)
